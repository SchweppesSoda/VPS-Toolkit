...
This release uses script version `2026.09.05+build.4` and tag `po0-v2026.09.05.4`; the outbound APK is `2026.09.05-r2` and the WAN probe APK remains `2026.08.30-r5`. Legacy raw executable entry points are disabled. Egern, Stash, Loon, and independent tools not included in the Release continue to use the allowed raw paths documented by their own guides.
