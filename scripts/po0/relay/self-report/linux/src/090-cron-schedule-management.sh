...
install_cron() {
    ...
    apply_channel_cron install "$channel" || { self_report_incomplete '定时任务安装失败。'; return 1; }
    self_report_completed '自建与官方按各自独立任务执行。'
    show_cron_status "$channel"
}

remove_cron() {
    apply_channel_cron remove "${1:-${SCHEDULE_CHANNEL:-all}}" || return 1
    self_report_completed '已删除所选通道的定时任务，保存配置保留。'
}
