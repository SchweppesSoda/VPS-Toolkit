...
脚本版本统一采用 `YYYY.MM.DD+build.N` 混合版本格式；正式 PO0 Release asset 的五个脚本版本必须与 release tag 尾号一致，例如 `po0-v2026.07.01.7` 对应 `2026.07.01+build.7`。PO0 manager 的 `--version` 会像 LAN Worker 一样单独显示 build 构建标识。
...
OpenWrt APK 通过 `runtime-manifest.txt` 复用 Linux 的探测、SSID guard 和 Worker 提交核心；调度由 APK 自己维护，不加载 Linux 的安装、cron 和自更新入口。`procd` 分别注册 `worker` / `official` 实例，周期来自 UCI `interval_seconds` / `official_interval_seconds`（默认 3600 / 600 秒，0 关闭该定时）。`worker_network_enabled` / `official_network_enabled` 默认启用，只控制对应通道的网络触发。原配置缺少新字段时采用默认值，既有总开关和通道停用状态保持不变。配置 reload 以各实例的周期数据判断是否重启，不因另一通道修改周期而重启本通道。

`iface` hotplug 接受 `ifup` 和含 `IFUPDATE_ADDRESSES=1` 或 `IFUPDATE_ROUTES=1` 的 `ifupdate`，每通道单独调用 service 的一次性 `network` 入口；本机 WAN 模式按选择/绑定过滤，source/上游探测模式只响应本机变化，无法观测上游 WAN 事件。事件遵守自动开关和 Worker SSID guard，不调用手动强制参数。UCI 适配器按 `worker.lock` / `official.lock` 互斥，同通道 busy 返回 75 并由调度器重试，另一通道可独立运行。网络结果写入各通道 `.state` / `.log`，定时另存 `.timer.state`，避免网络结果挪动定时；Worker 延续完成时间计时，官方延续尝试时间计时。官方仍先 GET、必要时 POST。专项门禁为 `test-openwrt-service.sh`、`test-openwrt-hotplug.sh`、`test-openwrt-official-adapter.sh`，由 APK layout 门禁统一调用。
...
Self-report 后端同时提供 Stash 专用 `POST /stash-report/v1`。Stash 接口不接受 query token 或 `X-PO0-Token`，只接受与旧入口共用的 `SELF_REPORT_SECRET` Bearer；旧 `/report` 继续兼容 query token、`X-PO0-Token` 和 Bearer 三种提交方式，但两个入口在 secret 未配置时都会返回 503。服务安装会复用 settings 或旧 systemd unit 回填的已有 secret，仅在确实缺失时生成并保存新值。JSON 必填字段为 `source_id`、公网 IPv4 `ip`、`network`、`observed_at`、`request_id`，其中 source ID 必须符合 `[A-Za-z0-9][A-Za-z0-9._-]{0,47}`，network 只允许 `cellular`、`wifi`、`unknown`，时间漂移上限为 600 秒，请求 ID 在同一 server 进程内防重 600 秒。无论客户端报告蜂窝、Wi-Fi 还是未知网络，Stash 请求调用 manager 时都追加前缀 `32`，避免客户端网络探测失败扩大放行范围；旧 `/report` 调用仍省略前缀，保持 `/32` 和旧文本返回兼容。新入口以 JSON 返回 `ok`、`source_id`、归一化后的 `accepted_cidr`、`accepted_at`、`expires_at`、`targets` 和 `request_id`；任一 PO0 目标失败仍返回 502，但响应不回显 SSH 错误或 token，目标级细节只写 LAN Worker journal。Caddy 托管 snippet 仅反代 `/stash-report/v1`、`/report`、`/health`，其它路径仍返回 404。
...
Egern 官方目标在 PO0_FIREWALL_TOKENS 中同时兼容旧 Token 列表和 `Token@slot|name|interval=N|timer=true/false` 行。扩展行按换行 / 逗号 / 分号分隔以保留名称空格，纯 Token 行继续按空白分隔。保存时名称拆入 PO0_FIREWALL_NAMES，Token 行保留槽位和定时选项，使旧名称单独修改 / 清空仍有效；行内名称优先并按 Token 身份匹配。每账号独立持久化 lastAttemptAt；缺少字段的旧状态回退到通道 lastAttemptAt，新增 / 换槽账号立即检查；只读状态不改变这些时间，某账号的检查不得推进其它账号的周期。全局官方定时开关优先于各行 timer；网络和手动入口绕过定时 due，保留自动开关与 SSID guard。interval 仅用于本地调度，不向 API 发送；TTL、未知或重复选项明确拒绝。
