...
如果结果显示 `redacted` / `<redacted>` 或提示被 macOS 隐私权限隐藏，说明当前终端或 launchd 运行环境没有拿到系统允许的定位相关信息访问权；客户端会按 fail-open 继续正常上报，不会把占位符当成真实 SSID。查看授权说明：
