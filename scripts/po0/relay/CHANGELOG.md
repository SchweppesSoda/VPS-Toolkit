...
## 2026-09-07 Egern 本机目标与运行参数分开

- 自建目标、SSH 认证、逐目标 TTL、官方 Token / 槽位继续保存在本机，运行时不会采用同步来的自建目标或认证。
- 两通道上报间隔、定期开关、按网络选择开关、SSID、通知及出口探测设置直接读取当前模块参数，无需保存；官方旧目标时间列不再控制运行。
- 取消通用保存入口；配置总览和组件共用实际运行参数，忙碌时显示也保持一致。

## 2026-09-07 Egern 小组件周期显示

- 官方逐账号显示已保存的实际间隔，自建与官方显示定时关闭状态。
- 小组件刷新请求时间跟随已开启通道的最短间隔；没有启用的定时通道或 SSID 跳过时不指定刷新时间。

## 2026-09-07 脚本 build.2 / OpenWrt APK r2：收回遗漏修复

- Windows 网络监听的登录触发器限定当前用户，修复普通权限注册任务时的“拒绝访问”。
- 自动开关只启停已有定时和网络监听任务；失败恢复原开关和任务状态，保留新版通道菜单及独立定期开关。
- OpenWrt 槽位下拉选项和状态表同时标注“槽位 1（@0）”至“槽位 5（@4）”；明确自动槽位不等于 @0。
- 修复 LuCI 密码输入框与显示按钮的横向布局，更新页面缓存版本。

## 2026-09-07 手机客户端按网络选择官方目标

- Egern、Loon、Stash 增加默认关闭的按网络选择功能：原目标用于蜂窝，Wi-Fi 使用另填目标，沿用原目标语法和自填槽位。
- 两套目标与开关保存到本机，跨网络不串用上报间隔；只读查询不推进上报时间。Stash 通过互补策略探测区分网络，未知时不发送官方请求。
- 官方配置和说明统一使用“上报间隔”，删除官方白名单 TTL 误述；自建 TTL 和上报间隔保留。

## 2026-09-07 七端统一上报设置（脚本 2026.09.07+build.1 / APK 2026.09.07-r1）

- Windows、macOS、Linux、Egern、Loon、Stash、OpenWrt 统一自动上报、定期开关、上报间隔与白名单有效期（TTL）的名称和含义。保持平台默认间隔及已有配置。
- 自建和官方分别保存、启停、清除和管理任务；修复 Loon / Windows 自建参数错误阻断官方的问题。支持仅配置官方及逐步淘汰自建。
- 桌面三端菜单对齐；Loon / Stash / OpenWrt 补充独立定期开关，关闭保留间隔，旧 0 保持原停用语义。
- 保存、本机配置与最近结果不隐含上报；官方查询只读。Egern 独立保存通用参数，小组件继续执行上报并刷新，官方目标第三列改称上报间隔。
- 移动模块更新缓存版本；五个脚本统一脚本版本，OpenWrt APK 独立版本和发布流程。Egern 同网段复用 / HTTP 403 修复保留前一独立提交。

## 2026-09-07 Egern：同出口网段复用与 403 诊断

- 官方按 /24 判断已放行；同网段位于其他槽位时显示共用，避免新设备重复占槽触发 403，配置槽位仍供新网段使用。
- POST 403 后仅追加一次只读确认，确认覆盖才算成功；未确认时保留失败，并区分查询和加白阶段。不删除槽位、不重试写入、不记录原始错误正文。
- 自建选项改称“启用定期上报”和“定期上报间隔”，说明关闭后只保留网络变化触发，间隔暂不使用；设置总览显示开关。保留白名单有效期说明，模块缓存版本更新为 20260907-shared-network。

## 2026-09-07 Egern：移除重复的官方名称输入框

- 官方名称统一在目标第二列填写，移除原独立“目标名称”表单项；旧模块和本机保存的名称继续读取，更新不会丢失。

## 2026-09-07 Egern：简化官方目标 TTL 填写

- 按用户要求改为 `Token@槽位|名称|TTL秒数`，直接写数字，不需要额外填写周期参数名或定时开关。
- TTL 在此表示客户端上报周期：留空沿用默认 600 秒，0 关闭此目标定时且保留网络变化检查；官方白名单有效期不变。
- 旧本机行内选项仍可读取，保存时转为数字格式；同步模块缓存版本为 `20260907-official-ttl`。

## 2026-09-07 Egern：官方目标单行配置

- 官方目标支持 `Token@槽位|名称|interval=600|timer=true`，一行设置名称、槽位与账号定时；旧 Token 列表和名称字段继续兼容。
- 每个账号独立记录检查时间，支持关闭单账号定时且保留网络变化触发；只读查询不推迟下一次定时。
- 保存、同步名称显示与本机参数保持隔离；非法、重复和未接入的 TTL 参数明确报错。
- 模块脚本缓存版本更新为 `20260907-official-targets`，标准与历史兼容副本同步。

## 2026-09-06 OpenWrt APK 2026.09.06-r1：独立任务与网络触发

- 自建和官方拆成两个后台任务与互斥锁，周期、暂停和网络变化开关分别控制。
- 官方定时默认 600 秒且可调整；两通道均可设为 0 仅关闭定时，网络变化继续上报。
- 监听本机接口上线、地址和路由变化；旁路网关明确提示无法直接监听上游 WAN 变化。
- 网络触发不改动下一次定时，保留旧配置和既有停用状态；更新 LuCI 缓存版本。

## 2026.09.06+build.4 上报任务与网络变化触发

- Windows / macOS / Linux：自建与官方分别安装定时任务，周期、暂停、删除和日志隔离；升级只刷新已有任务，旧共享任务迁移为两项。
- 官方周期可设置，默认 600 秒。菜单输入 0 关闭定时，网络变化监听可继续运行。保留旧配置的自建周期。
- Windows 使用 NetworkAddressChanged，macOS 使用 scutil 网络通知；Linux 仅在可用的 NetworkManager / OpenWrt 环境接入事件，不支持时只保留可选定时。
- Egern / Loon 保留已有原生网络变化入口；每个通道可选定时默认 600 秒。Stash 每分钟检查 DIRECT 出口 IPv4 变化，明确其轮询限制。未新增 Surge 支持。
- 增加任务迁移、关闭定时仍响应切网、自定义 900 秒周期回归。manager / LAN Worker 仅同步版本，本次不修改 APK。

## 2026-09-06 Egern：清理重复入口并补齐操作回执

- 模块由 21 个脚本入口精简为 18 个：移除旧“PO0 SSH 上报状态”和旧浏览器设备 ID 入口；“全部强制上报”合并到唯一的“PO0 防火墙上报状态”，单通道强制上报继续保留。
- 定时检查和网络变化检查分别标注“后台自动”，两个通道继续共用触发、SSID guard 和上报锁。
- 原生手动入口成功、失败、未配置、锁占用和存储异常都返回可显示的结果；修复单通道强制上报只返回普通对象的问题。
- 旧设备入口缺少或不匹配 HTTP 请求时不再误入上报；设备 ID、上报配置及旧自定义脚本的兼容代码继续保留。
- 按实际 YAML 入口和官方文档中的 ctx.script.name 模拟逐项回归，验证 16 个手动入口及两个后台入口。

## 2026.09.06+build.3 三端目标名称与有效期说明

- Windows / macOS / Linux：主菜单和官方通道页逐个显示已保存的官方目标名称，未命名账号显示默认编号。
- 自建有效期仅说明由 LAN Worker 接收端管理，移除菜单 / 本机状态中的默认 43200 秒展示。
- 名称保存、来源 ID / 备注、手动立即上报、只读状态与 SSID 跳过逻辑保持原行为。
- 使用 `po0-scripts-v2026.09.06.3` 独立发布五个主脚本及校验文件；manager / LAN Worker 仅同步版本，本次不构建 APK。

## 2026-09-06 Egern：修正目标名称与状态页强制刷新

- 官方目标名称在组件 / 状态页刷新时直接生效，按账号对应而不替换本机 Token / 槽位；支持仅填写名称后保存，结果显示实际保存名称和槽位。
- 自建组件与设置总览改用多目标第一列来源 ID，Token 后的备注继续用于上报审计。
- 去掉小组件中的自建 TTL 展示，目标内 TTL 和原上报协议保持不变。
- 修复普通状态页连续刷新时可能按周期跳过自建上报的问题；Widget 和状态页每次刷新均强制执行自建 SSH，官方仍先 GET、必要时 POST。

## 2026-09-06 Egern：统一小组件与目标参数

- 小 / 中 / 大号重新安排字体、间距和信息量；移除空文本行、重复统计和多余分隔，模块只保留一个小组件入口。
- 自建与官方采用相同的自动状态显示：自动开启、自动停用、SSID 跳过；SSID 命中时两条启用通道都跳过，手动强制刷新保持原行为。
- 官方目标显示当前本机保存名称；中号可直接看到两个账号名称，大号逐目标显示 TTL 或槽位 / 名额，不把第一条目标参数当作全局值。
- 设置总览逐目标显示生效 TTL；目标字段始终优先于旧默认，文本与 JSON 空值回退统一，周期与 TTL 分开说明。
