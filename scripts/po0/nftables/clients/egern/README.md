...
“PO0 防火墙上报状态”是唯一状态小组件，也可手动上报并刷新。`20260909-ssid-guard-v1` 按尺寸和账号数量调整字号，将剩余高度分配给账号内容，底部显示最近检查时间。

- 小尺寸显示前两个账号，放大出口、名称、槽位和占用；单账号补充上报时间，顶部显示本机网络。
- 中尺寸左侧显示出口、网络及自动检查方式，右侧显示前三个账号；一至两个账号使用较大字号，单账号补充逐行白名单摘要。
- 大尺寸一个账号时使用全宽卡片，两个账号时并排展示；两种情况均逐槽位换行显示完整白名单（最多五槽），当前覆盖槽位标绿。三至四个账号各显示两条白名单，五至六个账号各显示一条，按排序取前几条并标明省略数量。失败时显示原因；完整查询入口也按条目换行。

槽位与占用来自官方返回值。白名单统一按 `#1、#2、#3、#4、#5` 升序排列，未标明槽位的记录显示为“自动”并放在最后；当前覆盖项只标绿，不置顶。共用网段显示实际覆盖槽位，自动分配成功后显示实际槽位；官方未标明槽位的记录不会被误画成空槽。

超出尺寸容量时显示剩余账号数，完整列表可通过“查询官方白名单”查看。无账号时显示设置指引；请求未取得名额数据时显示未知占用，避免将失败误读为零名额。更新模块获取新的脚本版本后刷新组件即可；组件名称和本机配置继续沿用。

“查询官方白名单”为只读入口，不 POST，也不推进下次定期检查时间。后台定时与网络变化按开关和各账号 due 分别判断。定时、网络变化、立即上报、上报状态和 Widget 刷新统一遵守当前模块参数中的 SSID 跳过：命中时不 GET、不 POST，显示“SSID 跳过 · 本次未上报”，保留之前结果和账号 due。多个 SSID 用英文分号分隔并精确匹配；读取不到 SSID 时继续。只有手动执行“仅官方防火墙强制上报”可绕过 SSID，普通刷新不等于强制上报。已有白名单记录不会因本次跳过而删除。
