...
“PO0 防火墙上报状态”是唯一状态小组件，也可手动上报并刷新。`20260908-official-widget-v2` 使用深色卡片布局，顶部显示放行或异常数量，底部显示最近检查时间。

- 小尺寸显示前两个账号，保留名称、结果、槽位、名额占用、出口 IP 与本机网络。
- 中尺寸左侧显示出口、网络及自动检查方式，右侧显示前三个账号。
- 大尺寸最多显示六个账号，各自显示出口与上报时间；一个账号时逐项显示五个白名单槽位，两个账号时展开白名单摘要，失败时显示原因。共用网段显示实际覆盖槽位，自动分配成功后显示实际槽位。
