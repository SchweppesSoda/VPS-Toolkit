...
更新模块 YAML 后，新脚本 URL 的版本参数会让模块引用本次修复版本；也可主动刷新脚本。保留旧名称“PO0 SSH 上报状态”及其脚本绑定，已添加到桌面的旧小组件继续运行相同逻辑。新建组件选择“PO0 防火墙上报状态”；若桌面原组件仍提示找不到入口，长按它并在“编辑小组件”中重新选择此名称。
...
来源 ID、主机和 Token 填入对应列；端口、用户、脚本路径、备注和 TTL 可以留空，默认分别为 `22`、`root`、`/root/nftables-relay-manager.sh`、`egern` 和 `43200` 秒。最简一条可写：
...
- `PO0 防火墙上报状态` / `widget`：显示本机设备 ID、公网 IP、上报 CIDR、IP 归属地、运营商、自动上报周期、每个 PO0 target 的成功/失败，以及官方防火墙各账号的当前出口、白名单、`已用/上限`、当前命中条目和固定槽位。点击“更新小组件”或刷新普通上报状态时，官方通道也会先 GET，缺失或固定槽位不符时 POST；本机 600 秒间隔和 SSID guard 不会阻止本次刷新。只有“PO0 官方防火墙状态（只读）”保留只读检查；归属地 / 运营商优先使用本次 IP 查询接口返回的数据，拿不到时才额外查询。
