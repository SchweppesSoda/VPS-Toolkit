...
3. 打开 `PO0 防火墙上报状态`，确认显示 `设备: <device-id>`。
...
这些 URL 是普通 HTTP request 脚本拦截；如果设备会强制升 HTTPS，就使用上面的 `DEVICE_ID_SETUP` + `保存本机设备 ID` 流程。设备 ID 不会动态显示在模块设置表单里；请在 Egern 的 `PO0 防火墙上报状态` / Widget 里查看 `设备: ...`。

## 小组件更新与故障提示

更新模块 YAML 后，新脚本 URL 的版本参数会让模块引用本次修复版本；也可主动刷新脚本。模块只提供一个小组件“PO0 防火墙上报状态”。旧 generic 脚本名继续兼容已有自定义脚本绑定，但不再额外注册旧名称的小组件。曾选择旧小组件名称的桌面组件，请在“编辑小组件”中改选唯一的新入口；无需删除上报配置。
...
- `network`：网络变化时触发一次。
- `强制上报 PO0 防火墙`：立即执行已配置的自建和官方通道，绕过本机自动周期及 SSID guard；官方仍先 GET，缺失或固定槽位不符才 POST。
...
- `PO0 防火墙上报状态` / `widget`：按尺寸展示公网出口、目标名称、两条通道的自动状态和最近结果；大号补充官方槽位 / 名额。完整白名单由官方只读状态入口展示。点击“更新小组件”或刷新普通上报状态时，官方通道也会先 GET，缺失或固定槽位不符时 POST；本机 600 秒间隔和 SSID guard 不会阻止本次刷新。只有“PO0 官方防火墙状态（只读）”保留只读检查；归属地 / 运营商优先使用本次 IP 查询接口返回的数据，拿不到时才额外查询。
