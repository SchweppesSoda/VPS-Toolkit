...
小、中、大号分别安排信息：小号每通道显示一个目标及其他目标数量；中号两列展示，每通道最多两个目标；大号每通道最多三个目标，并逐目标显示 TTL 或官方槽位 / 名额。名称使用本机当前保存值，改名后不会继续显示旧缓存名。结果与对应账号 / 槽位匹配，换 Token、换槽位或旧缓存无法确认归属时显示待检查；账号重排不会串用结果。较多目标以数量提示，完整目标和生效 TTL 可在“查看本机上报设置”核对，官方白名单详情在独立只读状态入口查看。
...
只导入一份模块，统一在“自建防火墙上报目标”（`SSH_REPORT_TARGETS`）填写；一台服务器填写一条，多台用换行、逗号、分号或空格分隔。第一列 `source-id` 是自建防火墙的来源 ID；`source-id` 和备注 `identity` 支持 `{device}` 占位符：
...
- `PO0 防火墙上报状态` / `widget`：按尺寸展示公网出口、目标名称、两条通道的自动状态和最近结果；大号补充各目标的 TTL 或官方槽位 / 名额。完整白名单由官方只读状态入口展示。点击“更新小组件”或刷新普通上报状态时，官方通道也会先 GET，缺失或固定槽位不符时 POST；本机 600 秒间隔和 SSID guard 不会阻止本次刷新。只有“PO0 官方防火墙状态（只读）”保留只读检查；归属地 / 运营商优先使用本次 IP 查询接口返回的数据，拿不到时才额外查询。
