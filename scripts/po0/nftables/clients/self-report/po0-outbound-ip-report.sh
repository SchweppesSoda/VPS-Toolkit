...
SCRIPT_VERSION="2026.06.25+build.5"
...
usage() {
    printf '%s\n' \
        ...
        "  curl -fsSL ${DOWNLOAD_URL} | bash -s -- --worker-url https://report.example.com/report --secret SECRET --save-config --menu" \
        ...
        "  --install-cron [N]    安装 / 更新定时上报；Linux/OpenWrt 用 cron，macOS 缺少 crontab 时自动用 launchd。N 为兼容分钟参数，不带 N 时默认 3600 秒。" \
        "  --pause-schedule      暂停本脚本管理的定时上报；手动立即上报仍可用。" \
        "  --resume-schedule     恢复本脚本管理的定时上报。" \
        "  --schedule-status     查看本脚本管理的定时上报状态。" \
        "  --interval-seconds N  设置定时上报间隔秒数，必须是 60 的倍数，默认 3600。" \
