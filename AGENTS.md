...
- Egern 原生动作使用“启用 / 停用自动上报”，明确只控制定时与网络变化，重复点击保持指定状态；旧切换动作仅做兼容。按用户要求模块只注册一个“PO0 防火墙上报状态”小组件，旧 generic 脚本绑定保留，不能为名称兼容额外注册第二个小组件；小组件 / 状态入口遇到 busy 或存储异常也必须返回 Widget DSL，不能返回普通上报对象。
