...
- Self-report 是功能 / 协议 / 菜单 / 日志名；`po0-outbound-ip-report*` 是访问设备客户端 Release asset 和固定脚本文件名。Windows 旧 `po0-self-report.ps1` / `po0-self-report-task.vbs` 只做 legacy 迁移和卸载，不再作为默认安装路径。
