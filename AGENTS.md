...
- macOS 当前 Wi-Fi SSID 读取遇到 `redacted` / `<redacted>` 时应按系统隐私权限隐藏处理，必须 fail-open 继续上报；只允许提供 `--show-wifi-ssid` / `--diagnose-wifi-ssid` 这类本地诊断和用户授权指引，不自动获取或修改定位服务 / TCC 权限，不运行 `sudo`、不保存提权凭据、不写 TCC 数据库。
