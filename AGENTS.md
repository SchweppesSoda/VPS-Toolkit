...
- Egern 官方目标使用 `Token@槽位|名称|TTL秒数`，时间直接填数字，留空沿用模块默认（默认 600），0 仅关闭此目标定时；无需让用户填写 interval 或 timer 参数名。这里 TTL 明确标为客户端上报周期，不伪称官方白名单有效期；自建 SSH 的 TTL 有效期含义保持原样。旧 Token / 名称字段及旧本地时间选项继续兼容，保存时规范成纯数字列。名称只用于本地显示；定时按账号和槽位独立计时，只读查询不推进周期，保留自动开关、SSID guard 和 GET-first / POST 协议。
