...
- Egern 官方目标支持 `Token@槽位|名称|interval=600|timer=true`，旧 Token / 名称字段继续兼容；行内名称优先且仅用于显示，持久化时与凭据分开。定时按账号和固定槽位记录 lastAttemptAt，旧状态回退读取通道时间；只读查询不推进周期。官方定时总开关关闭所有账号定时，网络事件仍受自动开关 / SSID guard 控制。未知、重复或非法参数及 TTL 必须报错，不接受后静默忽略；新格式不能改变官方 GET-first / POST 协议。
