...
- Egern 小组件按尺寸控制文字与行数，两条通道均从当前保存配置计算自动开关和共享 SSID guard，最近结果与自动状态分开表达；SSID 命中不是持久停用。自建标签使用第一列 source-id，不能用 Token 后的 identity 备注；小组件不展示自建 TTL。官方名称属于显示项，优先读取当前模块填写的非空名称并按 Token 身份映射到本机账号，模块 Token 为空时按本机顺序；未填名称或无法对应时沿用本机名称，不改动已保存 Token / 槽位。官方保存支持 Token 留空时仅更新名称并回显对应名称 / 槽位。普通状态页与 Widget 连续刷新都必须强制执行 SSH，不能被 unchanged/due 检查跳过。目标字段优先于所有旧 env 别名，留空才回退到旧默认 / 内置默认；设置总览展示每个目标的生效 TTL，不把某一目标 TTL 或槽位显示为全局参数。
- Egern 官方目标使用 `Token@槽位|名称|上报间隔秒数`，时间直接填数字，留空沿用模块默认（默认 600），0 仅关闭此目标定时；无需让用户填写 interval 或 timer 参数名。第三列明确标为上报间隔，TTL 一律只指白名单有效期；自建 SSH 最后一列的 TTL 含义保持原样。旧 Token / 名称字段及旧本地时间选项继续兼容，保存时规范成纯数字列；官方名称只在目标第二列填写，不能在 env_schema 恢复独立 PO0_FIREWALL_NAMES 表单项，已有名称存储与旧模块读取保留。名称只用于本地显示；定时按账号和槽位独立计时，只读查询不推进周期，保留自动开关、SSID guard 和 GET-first / POST 协议。
...
- 保存配置、查看本机配置、查看最近结果不联网提交；查询官方白名单只发 GET；立即上报无需等间隔，强制上报绕过本机跳过条件。Egern 只有一个 Widget，明确为“上报并刷新”。Egern 通用参数有独立保存入口，不得要求仅官方设备先设置自建。
