...
- Egern 小组件按尺寸控制文字与行数，两条通道均从当前保存配置显示名称、自动开关和共享 SSID guard，最近结果与自动状态分开表达；SSID 命中不是持久停用。目标字段优先于所有旧 env 别名，留空才回退到旧默认 / 内置默认；设置总览展示每个目标的生效 TTL，不把某一目标 TTL 或槽位显示为全局参数。
