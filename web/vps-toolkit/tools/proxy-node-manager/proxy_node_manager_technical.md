...
- 支持节点 URL/URI 链接在本地浏览器生成二维码，可从手动输入或节点表格的原始/转发链接载入。
...
- 中部结果区：清洗出结果后，每个分组用同一个组头承载“VPS 前缀与节点分组”说明、VPS 前缀、分组依据、节点统计、入口统计、协议 badges 和组操作按钮；不要再把分组说明拆成独立标题块。
- 底部输出区：`转发与最终输出` 标题行只放标题说明和主要动作；输出内容 tabs、转发输出组合和输出格式放在下一行 toolbar，避免标题栏拥挤。
- 节点二维码浮窗：通过右侧悬浮入口或节点表格行按钮打开，粘贴或载入一条 URL/URI 节点链接后生成二维码并下载 PNG。
...
- 手动粘贴单条 URL/URI 节点链接。
- 点击节点表格“原始”，把该行 URL/URI 原始链接载入二维码浮窗；Clash proxy 块不是 URL/URI 链接，按钮会禁用。
- 点击节点表格“转发”，按当前行转发入口、端口和转发名称生成转发节点链接并载入二维码浮窗；未配置有效转发或无法生成 URL/URI 时按钮会禁用。
...
修改 UI 布局后，还应检查桌面和小屏视口：分组组头应把说明、VPS 前缀和分组依据放在同一元素内；输出区应保持标题/动作与输出 toolbar 分行；结果表二维码列、行转发输入和最终输出不能回归。
...
- 节点二维码浮窗能手动生成、从节点表格原始 URL/URI 链接或有效转发 URL/URI 链接生成，并能下载 PNG。
- 桌面宽度下，分组说明、VPS 前缀、分组依据、统计 badges 和组操作在同一组头中；输出区标题/动作与 tabs/select toolbar 分行展示。
- 小屏宽度下，分组组头、输出 toolbar、设置区控件不互相挤压，页面无整体横向溢出。
